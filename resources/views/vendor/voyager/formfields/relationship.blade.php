@if(isset($options->model) && isset($options->type))
    ...
    @if(class_exists($options->model))
        ...
        @elseif($options->type == 'belongsToMany')
                        {{-- polymorphic relationship extend cheat --}}
                    @if ( Str::contains($options->pivot_table , 'ables') )
                                            ...
                                            @php
                                                ...
                                                $selected_values = isset($relationshipData) ? $relationshipData->morphToMany($options->model,  Str::singular($options->pivot_table)  , $options->pivot_table,  $options->foreign_pivot_key ?? 'taggable_id', $options->related_pivot_key ?? null, $options->parent_key ?? null, $options->key)->get()->map(function ($item, $key) use ($options) {
                                                    return $item->{$options->label};
                                                })->all() : array();
                                            @endphp
                                        ...
                                        @endif
                    @else
                                            {{--END !! polymorphic relationship extend cheat --}}
